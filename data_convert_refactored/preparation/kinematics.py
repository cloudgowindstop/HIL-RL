"""离线转换使用的轻量URDF FK，不依赖ROS或实时机器人环境。"""
...
class URDFKinematics:
    """解析一条base到末端的URDF链，并应用关节符号、零偏和TCP变换。"""
    ...
    def __init__(self, config: dict, config_dir: Path):
        ...
        self.joint_signs = np.asarray(
            config.get("joint_signs", [1.0] * count), dtype=np.float64
        )
        self.joint_offsets = np.asarray(
            config.get("joint_offsets", [0.0] * count), dtype=np.float64
        )
        if self.joint_signs.shape != (count,) or self.joint_offsets.shape != (count,):
        ...
        while link != self.base_link:
            if link not in child_to_joint:
                raise ValueError(
                    f"no URDF chain from {self.base_link} to {self.end_effector_link}; "
                    f"stopped at {link}"
                )
        ...
        chain_movable = [
            ...
            if joint.kind in ("revolute", "continuous", "prismatic")
        ]
...
__all__ = ["DualArmKinematics", "URDFKinematics"]
