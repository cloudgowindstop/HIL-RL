...
def resolve_actor_pretrain_pth(load_path: str) -> str:
    """Resolve learner-style actor .pth path (same as learner_copy_dist).

    - ``.pth`` file → use as-is
    - directory → pick max ``actor_pretrain_step{N}.pth``, else ``actor_pretrain.pth``
    """
    path = os.path.abspath(os.path.expanduser(str(load_path)))
    if os.path.isfile(path):
        if not path.endswith(".pth"):
            raise ValueError(f"Expected a .pth file, got: {path}")
        return path
    if not os.path.isdir(path):
        raise FileNotFoundError(f"load_path not found: {path}")

    step_ckpts = []
    for name in os.listdir(path):
        if name.startswith("actor_pretrain_step") and name.endswith(".pth"):
            try:
                step = int(name[len("actor_pretrain_step") : -len(".pth")])
                step_ckpts.append((step, name))
            except ValueError:
                continue
    if step_ckpts:
        return os.path.join(path, max(step_ckpts)[1])

    fallback = os.path.join(path, "actor_pretrain.pth")
    if os.path.isfile(fallback):
        return fallback
    raise FileNotFoundError(
        f"No actor_pretrain_step*.pth or actor_pretrain.pth under {path}"
    )


def detect_checkpoint_kind(load_path: str) -> str:
    """Return ``'pth'`` or ``'safetensors'`` based on load_path contents."""
    path = os.path.abspath(os.path.expanduser(str(load_path)))
    if os.path.isfile(path):
        ...
        if path.endswith(".safetensors"):
            return "safetensors"
        raise ValueError(f"Unsupported checkpoint file type: {path}")

    if not os.path.isdir(path):
        raise FileNotFoundError(f"load_path not found: {path}")

    has_step_pth = any(
        n.startswith("actor_pretrain_step") and n.endswith(".pth") for n in os.listdir(path)
    )
    has_plain_pth = os.path.isfile(os.path.join(path, "actor_pretrain.pth"))
    has_safetensors = os.path.isfile(os.path.join(path, "model.safetensors"))

    # Prefer explicit pth if present (cosmos offline ckpts)
    if has_step_pth or has_plain_pth:
        return "pth"
    if has_safetensors:
        return "safetensors"
    raise FileNotFoundError(
        f"No actor_pretrain*.pth or model.safetensors under {path}"
    )


def load_policy_from_path(policy: nn.Module, load_path: str, device: torch.device) -> str:
    """Load weights into policy; auto-select pth vs safetensors. Returns resolved path."""
    kind = detect_checkpoint_kind(load_path)
    print(f"checkpoint kind: {kind}")

    if kind == "pth":
        # Same as learner_copy_dist: only actor/net weights
        pth_path = resolve_actor_pretrain_pth(load_path)
        print(f"======================>>> loading actor pth: {pth_path}")
        state_dict = torch.load(pth_path, map_location=device)
        # if any(k.startswith("module.") for k in state_dict):
        #     # 不存在module前缀，是否需要移除module前缀的键
        #     state_dict = {k.removeprefix("module."): v for k, v in state_dict.items()}
        policy.actor.load_state_dict(state_dict, strict=True)
        return pth_path

    # Original safetensors path (full policy)
    if os.path.isfile(load_path) and str(load_path).endswith(".safetensors"):
        safetensors_path = load_path
    else:
        safetensors_path = os.path.join(load_path, "model.safetensors")
    print(f"======================>>> loading safetensors: {safetensors_path}")
    state_dict = load_file(safetensors_path)
    policy.load_state_dict(state_dict, strict=False)
    if hasattr(policy, "actor") and hasattr(policy.actor, "encoder"):
...
def act_with_policy(
    ...
    try:
        ...
        load_path = getattr(env_cfg, "load_path", None)
        print(f"load_path: {load_path}")
        if load_path is not None and str(load_path) not in ("None", "null", ""):
            try:
                resolved = load_policy_from_path(policy, load_path, device)
                env_cfg.load_path = resolved  # for eval_success naming
            except Exception as e:
    ...
    for interaction_step in range(cfg.policy.online_steps):
        ...
        with policy_timer:
            ...
            if env_cfg.policy_type != "cosmos":
                ...
                policy_obs = cosmos_obs_to_transition_state(obs)
        ...
        next_obs, reward, terminated, truncated, info = online_env.step(policy_action)
        ...
        if done:
            ...
            print_green(f'================success/episode: {success_episode}/{episode}================')
