...
try:
    from xhitl.rl_env.utils.film_conditioning_layer import FilmConditioning
    from xhitl.rl_env.utils.data_augmentations import resize
except Exception as e:
    from utils.film_conditioning_layer import FilmConditioning
    from utils.data_augmentations import resize
...
class ResNetEncoder(nn.Module):
    ...
    def __call__(
        ...
        x = observations.astype(self.dtype)
        # put inputs in [-1, 1]
        ...
        for i, block_size in enumerate(self.stage_sizes):
            for j in range(block_size):
                ...
                if self.use_multiplicative_cond:
                    ...
                    if self.use_lora:
                        cond_out = nnx.LoRALinear(cond_var.shape[-1], x.shape[-1], lora_rank=self.lora_rank, rngs=nnx.Rngs(0))(cond_var)
                    else:
                        cond_out = nn.Dense(
        ...
        if self.bottleneck_dim is not None:
            if self.use_lora:
                x = nnx.LoRALinear(x.shape[-1], self.bottleneck_dim, lora_rank=self.lora_rank, rngs=nnx.Rngs(0), dtype=self.dtype)(x)
            else:
...
class PreTrainedResNetEncoder(nn.Module):
    ...
    def __call__(
        ...
        if self.bottleneck_dim is not None:
            if self.use_lora:
                x = nnx.LoRALinear(x.shape[-1], self.bottleneck_dim, lora_rank=self.lora_rank, rngs=nnx.Rngs(0))(x)
            else:
