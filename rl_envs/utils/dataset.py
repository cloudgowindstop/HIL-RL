...
class Dataset(object):
    def __init__(self, dataset_dict: DatasetDict, seed: Optional[int] = None, action_mask: Optional[np.ndarray] = None):
    ...
    def sample(
        ...
        for k in keys:
            if isinstance(self.dataset_dict[k], dict):
                ...
                if k == "actions":
                    actions = self.dataset_dict[k][indx]
                    if self.action_mask is not None:
                        mask = self.action_mask
                        mask = np.broadcast_to(mask, actions.shape)
