...
from cosmos_policy.experiments.robot.libero.run_libero_eval import TaskSuite, TASK_MAX_STEPS
...
def act_with_policy(
    ...
    online_env = make_env(env_cfg, fake_env=True, use_human_intervention=env_cfg.use_human_intervention, classifier=True, use_gripper_penalty=cfg.policy.use_gripper_penalty, cfg=cfg)
    ...
    for interaction_step in range(cfg.policy.online_steps):
        ...
        if done:
            ...
            if len(list_transition_to_send_to_learner) > 0:
                ...
                global transitions_cnt
                global transitions_time
                transitions_cnt += len(list_transition_to_send_to_learner)
                transitions_time += push_transitions_end_time - push_transitions_start_time
                # print("--------------------------------- transitions_cnt:", transitions_cnt, "transitions_time:", transitions_time / transitions_cnt)
...
def update_policy_parameters(policy, parameters_queue: Queue, device):
    ...
    if bytes_state_dict is not None:
        # 测试 bytes_state_dict 大小：原始字节数 + 可读格式 (KB/MB)
        size_bytes = len(bytes_state_dict)
        size_mb = size_bytes / (1024 * 1024)
        logging.info("[ACTOR] Load new parameters from Learner. bytes_state_dict size: %d bytes (%.2f MB)", size_bytes, size_mb)
...
def _compute_transitions_image_bytes(transitions_list: list) -> int:
    """Compute total bytes of image tensors (observation.images.*) in state and next_state."""
    total_bytes = 0
    for tr in transitions_list:
        for state_key in ("state", "next_state"):
            state_dict = tr.get(state_key, {})
            for key, value in state_dict.items():
                if key.startswith("observation.images.") and isinstance(value, torch.Tensor):
                    print("--------------------------image size:", key, value.numel() * value.element_size() / (1024 * 1024))
...
def push_transitions_to_transport_queue(transitions: list, transitions_queue):
    ...
    # # 计算并记录传输的 image 总大小（state + next_state 中 observation.images.*）
    # image_bytes = _compute_transitions_image_bytes(transition_to_send_to_learner)
    # image_mb = image_bytes / (1024 * 1024)
    # logging.info(
    #     "[ACTOR] Push transitions: image data size %d bytes (%.2f MB), %d transitions",
    #     image_bytes, image_mb, len(transition_to_send_to_learner),
    # )

    # payload = transitions_to_bytes(transition_to_send_to_learner)
    # payload_mb = len(payload) / (1024 * 1024)
    # logging.info("[ACTOR] Push transitions: serialized payload size %d bytes (%.2f MB)", len(payload), payload_mb)
