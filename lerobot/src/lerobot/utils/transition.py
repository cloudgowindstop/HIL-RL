...
LATENT_STATE_KEY = "latent_state"


class Transition(TypedDict):
    state: Union[dict[str, torch.Tensor], torch.Tensor]
    action: torch.Tensor
    reward: float
    next_state: Union[dict[str, torch.Tensor], torch.Tensor]
    done: bool
    truncated: bool
    complementary_info: dict[str, torch.Tensor | float | int] | None = None


def _observation_to_state_dict(
    observation: Union[dict[str, torch.Tensor], torch.Tensor],
) -> dict[str, torch.Tensor]:
    if isinstance(observation, torch.Tensor):
        return {LATENT_STATE_KEY: observation}
    return observation
...
def move_transition_to_device(transition: Transition, device: str = "cpu") -> Transition:
    ...
    if isinstance(transition["state"], torch.Tensor):
        transition["state"] = transition["state"].to(device, non_blocking=non_blocking)
    else:
        transition["state"] = {
