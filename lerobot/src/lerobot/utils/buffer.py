...
class ReplayBuffer:
    ...
    def _initialize_storage(
        ...
        # }
        # 按照第一个transition的dtype分配内存，而不是按照shape分配内存(torch.empty()默认dtype是torch.float32)
        state_dtypes = {key: val.squeeze(0).dtype for key, val in state.items()}
        self.states = {
            key: torch.empty((self.capacity, *state_shapes[key]), dtype=state_dtypes[key], device=self.storage_device)
    ...
    def sample(self, batch_size: int, uniform_label_key: str | None = None) -> BatchTransition:
        ...
        # uniform_label_key：均衡采样
        if uniform_label_key is None:
            idx = torch.randint(low=0, high=high, size=(batch_size,), device=self.storage_device)
        else:
            # 根据 uniform_label_key 值按照8:2比例sample
            if uniform_label_key == "done":
                label_tensor = self.dones[:high].to(self.storage_device)
            else:
                raise ValueError(f"Unsupported uniform_label_key: {uniform_label_key}")
            ...
            if len(unique_labels) == 2:
                # 假设第一个标签是done=False(0)，第二个是done=True(1)
                false_indices = (label_tensor == 0).nonzero(as_tuple=True)[0]
                true_indices = (label_tensor == 1).nonzero(as_tuple=True)[0]
                
                # 计算8:2比例的数量
                false_samples = int(batch_size * 0.8)
                true_samples = batch_size - false_samples
                
                # 采样false样本
                if false_indices.numel() < false_samples:
                    false_sampled = false_indices[torch.randint(0, false_indices.numel(), (false_samples,), device=false_indices.device)]
                else:
                    false_sampled = false_indices[torch.randperm(false_indices.numel(), device=false_indices.device)[:false_samples]]
                
                # 采样true样本
                if true_indices.numel() < true_samples:
                    true_sampled = true_indices[torch.randint(0, true_indices.numel(), (true_samples,), device=true_indices.device)]
                else:
                    true_sampled = true_indices[torch.randperm(true_indices.numel(), device=true_indices.device)[:true_samples]]
                
                idx = torch.cat([false_sampled, true_sampled])
    ...
    def get_iterator(
        ...
        queue_size: int = 2,
        uniform_label_key: str | None = None,
    ):
        """
        ...
        while True:  # Create an infinite loop
            if async_prefetch:
                ...
                # 异步采样迭代器：用线程池提前预加载批次
                iterator = self._get_async_iterator(queue_size=queue_size, batch_size=batch_size, uniform_label_key=uniform_label_key)
            else:
                # 同步采样迭代器：训练时才采样，采样和训练串行
                iterator = self._get_naive_iterator(batch_size=batch_size, queue_size=queue_size, uniform_label_key=uniform_label_key)
    ...
    def _get_async_iterator(self, batch_size: int, queue_size: int = 2, uniform_label_key: str | None = None):
        """
        ...
        def producer() -> None:
            ...
            while not shutdown_event.is_set():
                try:
                    batch = self.sample(batch_size, uniform_label_key=uniform_label_key)
    ...
    def _get_naive_iterator(self, batch_size: int, queue_size: int = 2, uniform_label_key: str | None = None):
        """
        ...
        def enqueue(n):
            for _ in range(n):
                data = self.sample(batch_size, uniform_label_key=uniform_label_key)
                queue.append(data)
    ...
    def from_lerobot_dataset(
        ...
        Args:
            ...
            num_workers (int | None): Number of CPU threads for parallel dataset loading.
                None defaults to min(8, cpu_count). Set to 1 for sequential loading.
                This is multi-thread I/O parallelism, not multi-GPU.
        ...
        replay_buffer = cls(
            capacity=capacity,
            device=device,
            state_keys=state_keys,   # 这里的state_key
            image_augmentation_function=image_augmentation_function,
            use_drq=use_drq,
            storage_device=storage_device,
            optimize_memory=optimize_memory,
        )
        # 将LeRobotDataset转换为Transition列表：包含state action next.reward next.done episode_index complementary_info
        list_transition = cls._lerobotdataset_to_transitions(
            dataset=lerobot_dataset,
            state_keys=state_keys,
            num_workers=num_workers,
            skip_next_state=optimize_memory,
        )
        # Initialize the buffer with the first transition to set up storage tensors
        # 存储结构初始化，为第一个transition的state和action创建存储空间
        if list_transition:
    ...
    def to_lerobot_dataset(
        ...
        for key in self.states:
            ...
            if sample_val.ndim == 0:   # 针对value_function_return，转换成(1,)
                sample_val = sample_val.unsqueeze(0)
            if "proprio" in key:
                sample_val = sample_val.to(torch.float32)
        ...
        for idx in range(self.size):
            ...
            for key in self.states:
                val = self.states[key][actual_idx]
                if val.ndim == 0:
                    val = val.unsqueeze(0)
                if "proprio" in key:
                    val = val.to(torch.float32)
                frame_dict[key] = val.cpu()
    ...
    @staticmethod
    def _resolve_load_workers(num_workers: int | None) -> int:
        """Resolve CPU thread count for parallel dataset loading (not multi-GPU)."""
        if num_workers is None:
            return min(8, os.cpu_count() or 1)
        return max(1, int(num_workers))

    @staticmethod
    def _extract_state_from_sample(
        sample: dict[str, Any],
        state_keys: Sequence[str],
    ) -> dict[str, torch.Tensor]:
        """Build a state dict from a dataset sample (adds batch dim, casts proprio)."""
        state: dict[str, torch.Tensor] = {}
    ...
    @staticmethod
    def _load_dataset_samples_parallel(
        dataset: LeRobotDataset,
        num_frames: int,
        num_workers: int,
    ) -> list[dict[str, Any]]:
        """
        Load dataset[i] for all frames, optionally in parallel with ThreadPoolExecutor.

        Uses multi-thread CPU I/O parallelism (video/image decode), not multi-GPU.
        """
        samples: list[dict[str, Any] | None] = [None] * num_frames

        if num_workers <= 1 or num_frames <= 1:
            for i in tqdm(range(num_frames), desc="Loading dataset"):
                samples[i] = dataset[i]
            return samples  # type: ignore[return-value]

        print(
            f"Loading {num_frames} frames with {num_workers} CPU threads "
            f"(I/O parallelism, not multi-GPU)..."
        )
        # executor.map preserves order; chunksize reduces scheduling overhead
        chunksize = max(1, num_frames // (num_workers * 4))
        with ThreadPoolExecutor(max_workers=num_workers) as executor:
            for i, sample in enumerate(
                tqdm(
                    executor.map(dataset.__getitem__, range(num_frames), chunksize=chunksize),
                    total=num_frames,
    ...
    def _lerobotdataset_to_transitions(
        ...
        state_keys: Sequence[str] | None = None,
        num_workers: int | None = None,
        skip_next_state: bool = False,
    ) -> list[Transition]:
        """
        Convert a LeRobotDataset into a list of RL (s, a, r, s', done) transitions.

        Dataset frame loading is parallelized with ThreadPoolExecutor (CPU threads for
        I/O-bound video/image decode). This is not multi-GPU loading.

        Args:
            dataset (LeRobotDataset): The dataset to convert.
            state_keys (Sequence[str] | None): Keys for 'state' / 'next_state'.
            num_workers (int | None): CPU threads for parallel loading.
                None -> min(8, cpu_count). 1 -> sequential.
            skip_next_state (bool): If True (e.g. optimize_memory), skip building
                next_state from the next frame and reuse current_state.

        Returns:
            List[Transition]: Transitions with the same length as `dataset`.
        """
        ...
        workers = ReplayBuffer._resolve_load_workers(num_workers)
        print("num_frames", num_frames)
        print("num_episodes", num_episodes)
        print(f"dataset load workers: {workers}")

        # Parallel I/O: load all frames first, then assemble transitions in order
        samples = ReplayBuffer._load_dataset_samples_parallel(dataset, num_frames, workers)

        # Probe schema from first loaded sample
        sample0 = samples[0]
        has_done_key = "next.done" in sample0
        complementary_info_keys = [key for key in sample0 if key.startswith("complementary_info.")]
        ...
        for i in tqdm(range(num_frames), desc="Building transitions"):
            current_sample = samples[i]

            # 1. state
            current_state = ReplayBuffer._extract_state_from_sample(current_sample, state_keys)
            # 2. action
            ...
            if has_done_key:
                ...
                if not done and i < num_frames - 1:
                    if samples[i + 1]["episode_index"] != current_sample["episode_index"]:
                        done = True
            ...
            if (
                ...
                and not done
                and i < num_frames - 1
                and samples[i + 1]["episode_index"] == current_sample["episode_index"]
            ):
                next_state = ReplayBuffer._extract_state_from_sample(samples[i + 1], state_keys)
...
def concatenate_batch_transitions(
    ...
    if "is_intervention" in left_batch_transitions and "is_intervention" in right_batch_transition.keys():
        left_batch_transitions["is_intervention"] = torch.cat(
            [left_batch_transitions["is_intervention"], right_batch_transition["is_intervention"]],
            dim=0,
        )
    else:
        left_batch_transitions["is_intervention"] = None
