...
# 将param_groups转换为json安全的格式
def _make_param_groups_json_safe(param_groups: list[dict[str, Any]]) -> list[dict[str, Any]]:
    safe_groups = []
    for group in param_groups:
        safe_group = {}
        for key, value in group.items():
            if key == "params":
                safe_group[key] = [int(param_id) for param_id in value]
            elif isinstance(value, torch.Tensor):
                safe_group[key] = value.item() if value.dim() == 0 else value.detach().cpu().tolist()
            elif isinstance(value, tuple):
                safe_group[key] = [
                    v.item() if isinstance(v, torch.Tensor) and v.dim() == 0
...
# 恢复param_groups中的tensors
def _restore_param_groups_tensors(
    param_groups: list[dict[str, Any]], template: list[dict[str, Any]]
) -> list[dict[str, Any]]:
    for group, ref in zip(param_groups, template, strict=False):
        for key, ref_value in ref.items():
            if isinstance(ref_value, torch.Tensor):
                loaded_value = group[key]
                group[key] = (
                    torch.tensor(loaded_value, dtype=ref_value.dtype, device=ref_value.device)
                    if not isinstance(loaded_value, list)
                    else torch.tensor(loaded_value, dtype=ref_value.dtype, device=ref_value.device)
                )
    return param_groups
...
def _save_single_optimizer_state(optimizer: torch.optim.Optimizer, save_dir: Path) -> None:
    ...
    param_groups = _make_param_groups_json_safe(state.pop("param_groups"))
...
def _load_single_optimizer_state(optimizer: torch.optim.Optimizer, save_dir: Path) -> torch.optim.Optimizer:
    ...
    if "param_groups" in current_state_dict:
        ...
        param_groups = deserialize_json_into_object(
            ...
            _make_param_groups_json_safe(current_param_groups),
        )
        loaded_state_dict["param_groups"] = _restore_param_groups_tensors(param_groups, current_param_groups)
