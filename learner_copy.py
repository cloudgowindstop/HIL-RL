...
def add_actor_information_and_train(
    ...
    while True:
        ...
        # if cfg.policy.only_off_and_intervention:
            ...
            print("===============> sample from online iterator")
            batch = next(online_iterator)
            batch['is_intervention'] = batch["complementary_info"]["is_intervention"]
        
            if dataset_repo_id is not None:
                print("===============> sample from offline iterator")
                batch_offline = next(offline_iterator)
                batch_offline['is_intervention'] = torch.ones_like(batch_offline['done']).to(device)
                batch = concatenate_batch_transitions(
                    left_batch_transitions=batch, right_batch_transition=batch_offline
...
def cosmos_offline_training(
    ...
    if offline_replay_buffer is None:
        raise ValueError("cosmos offline training requires offline_replay_buffer")
    ...
    # )
    pretrain_dir = "/media/HIL-RL-Project/HIL-RL/experiments/pick_toy/exp_local/2026.07.31/cosmos_1"
    ...
    max_steps = 10000
    # Parse N from filenames like actor_pretrain_step5000.pth → [(5000, "actor_pretrain_step5000.pth"), ...]
    step_ckpts = []
    ...
    device = get_safe_torch_device(try_device=cfg.policy.device, log=True)
    start_step = 0
    print('step_ckpts:', step_ckpts)
    if step_ckpts:
        # max(...) by N → e.g. step5000 beats step1000; ckpt_step is that N
        ckpt_step, ckpt_name = max(step_ckpts)
        actor_pretrain_path = os.path.join(pretrain_dir, ckpt_name)
        print("actor_pretrain_path:", actor_pretrain_path, "ckpt_step:", ckpt_step)
        policy.actor.load_state_dict(torch.load(actor_pretrain_path, map_location=device), strict=True)
        print('Debug2: model parameters: ')
        # for n, p in policy.actor.named_parameters():
        #     print(n, p.mean().item())
        policy.actor.train()
        # Finished offline already → load and skip training loop
        if ckpt_step >= max_steps:
            print(f" ckpt_step={ckpt_step} >= max_steps={max_steps}, skip cosmos offline training")
            return
        # Partial ckpt → continue from next step
        start_step = ckpt_step + 1
        cosmos_training_step = start_step
    else:
        actor_pretrain_path = os.path.join(pretrain_dir, "actor_pretrain.pth")
        print("actor_pretrain_path:", actor_pretrain_path)
    ...
    trainer_config = TrainerConfig()
    print(
        f" >>> begin cosmos offline training from step {start_step}, "
        f"max_steps={max_steps}, cosmos_training_step={cosmos_training_step}"
    )
    for optimization_step in tqdm.tqdm(range(start_step, max_steps), desc="Cosmos offline training"):
        batch_offline = next(offline_iterator)
        batch_offline["is_intervention"] = torch.ones_like(batch_offline["done"]).to(device)
        batch = batch_offline
        actions = batch["action"]
        ...
        if grad_accum_iter == trainer_config.grad_accum_iter:
            ...
            if optimization_step % log_freq == 0:
                print(f"cosmos offline step {optimization_step}, loss_actor: {loss_actor.item():.4f}")
        ...
        if optimization_step % save_freq == 0 and optimization_step > 0:
            ...
            torch.save(policy.actor.state_dict(), actor_pretrain_path)
            print(f" success save actor pretrain model to {actor_pretrain_path} at step {optimization_step}")
...
def process_transitions(
    ...
    while not transition_queue.empty() and not shutdown_event.is_set():
        ...
        if env_cfg.policy_type == "cosmos":
            ...
            raw_encode_devices = os.environ.get("ENCODE_CUDA_DEVICES", "").strip()
            encode_device_ids = (
                [int(x.strip()) for x in raw_encode_devices.split(",") if x.strip() != ""]
                if raw_encode_devices
                else None
            )
