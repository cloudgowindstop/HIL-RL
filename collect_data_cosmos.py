...
def step_env_and_process_transition(
    ...
    if env_cfg.policy_type != "cosmos":
        ...
        obs = cosmos_obs_to_transition_state(obs)
...
def control_loop(
    ...
    # 将可能包含 PyTorch Tensor 或 NumPy 数组的复杂数据结构，转换成可以安全序列化（保存到磁盘）的格式
...
def encode_and_add_frame(
    ...
    for transition in transition_list:
        if dataset is not None and transition["complementary_info"]["is_intervention"]:
            ...
            # video_0 = state['video']
            # with open("collect_data_current_state_video_19.txt", "w", encoding="utf-8") as f:
            #     f.write(f"# shape={video_0.shape}\n")
            #     f.write(
            #         np.array2string(
            #             video_0,
            #             formatter={"float_kind": lambda x: f"{x:.3f}"},
            #             separator=", ",
            #             threshold=video_0.size + 1,
            #             max_line_width=120,
            #         )
            #     )
            #     f.write("\n")
            # print(f"collect_data_current_state_video_19.txt saved, shape={video_0.shape}")
            # exit(0)
...
def main(env_cfg):
    if "franka" in env_cfg.robot_config.robot_type:
        ...
        lerobot_config_path = "../../train_config_collect_data_tienyi.json"
        policy_config_path = "../../train_config_silri_tienyi.json"
    else:
    ...
    print("env_cfg.fake_env:", env_cfg.fake_env)
    try:
