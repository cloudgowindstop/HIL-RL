...
def load_initial_states(cfg: PolicyEvalConfig, task_suite, task_id: int, log_file=None):
    ...
    if cfg.initial_states_path != "DEFAULT":
        ...
        logging.info(f"Using initial states from {cfg.initial_states_path}")
        return initial_states, all_initial_states
    else:
        logging.info("Using default initial states")
...
def get_image_from_obs(obs, flip_images: bool = False):
    img = obs
...
def act_with_policy(
    ...
    if env_cfg.policy_type == "cosmos":
        ...
        # replay_path = "/media/HIL-RL-Project/cosmos-policy/cosmos_policy/LIBERO-Cosmos-Policy/success_only/libero_10_regen/LIVING_ROOM_SCENE2_put_both_the_alphabet_soup_and_the_tomato_sauce_in_the_basket_demo.hdf5"
        
        # if replay_path:
        #     logging.info("[REPLAY] replay_demo_hdf5_path=%s", replay_path)
        #     _run_cosmos_replay_demo_hdf5(cfg, cosmos_cfg, shutdown_event, task_suite, run_dir)
        #     return
    ...
    try:
        ...
        if env_cfg.policy_type == "cosmos":
            # 与 run_libero_eval.py 保持一致，直接用 cosmos_cfg 加载模型并挂载
    ...
    if env_cfg.policy_type == "cosmos":
        # todo3:reset
        # 保证初始状态的确定性
        if cosmos_cfg.deterministic_reset:
            reset_seed = cosmos_cfg.deterministic_reset_seed if cosmos_cfg.deterministic_reset_seed is not None else cosmos_cfg.seed
        ...
        initial_states, all_initial_states = load_initial_states(cosmos_cfg, task_suite, cosmos_task_id, None)
        cosmos_episode_idx = 1
        if cosmos_cfg.initial_states_path != "DEFAULT":
            # 设置为hdf5文件中指定轨迹的初始状态
            initial_states_task_key = task_description.replace(" ", "_")
            episode_key = f"demo_{cosmos_episode_idx}"
            logging.info(
                "Using initial state from JSON: task_key=%s episode_key=%s",
                initial_states_task_key,
                episode_key,
            )
            ...
            initial_state = initial_states[cosmos_episode_idx]

        if initial_state is not None:
            obs = online_env.set_init_state(initial_state)
        else:
    ...
    for interaction_step in range(cfg.policy.online_steps):
        ...
        with policy_timer:
            if "cosmos" not in env_cfg.policy_type:
                ...
                # reset
                if interaction_step < NUM_STEPS_WAIT:
                    obs, reward, done, info = online_env.step(get_libero_dummy_action(cosmos_cfg.model_family))
                    continue
               
                # 观测预处理
                # 原来和现在的policy_obs的形状和key分别是什么
                # 原来：dict_keys(['observation.state', 'observation.images.right', 'observation.images.wrist'])
                # 现在是{"primary_image": (1, 3, 224, 224), "wrist_image": (1, 3, 224, 224), "proprio": (1, 10)}
                policy_obs = prepare_observation(obs, resize_size, cosmos_cfg.flip_images)

                # print("observation shape: ", obs)
                # print("observation keys: ", obs.keys())
                # print("observation['agentview_image'].shape: ", obs["agentview_image"].shape)
                # print("observation['robot0_eye_in_hand_image'].shape: ", obs["robot0_eye_in_hand_image"].shape)
                # print("-----------------------------------------------------------------------")
                # print(f"policy_obs: {policy_obs}")
                # print(f"policy_obs['primary_image'].shape: {policy_obs['primary_image'].shape}")
                # print(f"policy_obs['wrist_image'].shape: {policy_obs['wrist_image'].shape}")
                # print(f"policy_obs['proprio'].shape: {policy_obs['proprio'].shape}")
                # exit(0)
                
                # replay_images.append(policy_obs["primary_image"])
                # if replay_wrist_images is not None:
                #     replay_wrist_images.append(policy_obs["wrist_image"])

                # 保存policy_obs的图片到指定路径
                if interaction_step % 30 == 0:
                    obs_img_dir = os.path.join(run_dir, "obs_images", f"step_{interaction_step:06d}")
                    save_image(policy_obs["primary_image"], os.path.join(obs_img_dir, "primary_image.png"))
                    if replay_wrist_images is not None:
                        save_image(policy_obs["wrist_image"], os.path.join(obs_img_dir, "wrist_image.png"))
            ...
            if interaction_step % 30 == 0 and last_best_future_predictions is not None:
            ...
            # # step之后的图片展示
            # next_obs = prepare_observation(next_obs, resize_size, cosmos_cfg.flip_images)
            # replay_images.append(next_obs["primary_image"])
            # if replay_wrist_images is not None:
            #     replay_wrist_images.append(next_obs["wrist_image"])
            # todo：保存step之后的图片到指定路径
            # 保存policy_obs的图片到指定路径
            # obs_img_dir = os.path.join(run_dir, "obs_images", f"step_{interaction_step:06d}")
            # print(f"----------------------------> obs_img_dir: {obs_img_dir}")
            
         
        # if interaction_step == 42:
        #     exit(0)
