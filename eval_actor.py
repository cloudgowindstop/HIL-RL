    ...
    try:
        ...
        if load_path is not None:
            try:
                ...
                #     policy.encoder_critic = policy.critic_ensemble._orig_mod.encoder

                # final_state_dict = policy.state_dict()  
                # for name, param in final_state_dict.items():
                #     if name in initial_state_dict.keys():
                #         print(f"name: {name} diff: {param.data.mean() - initial_state_dict[name].data.mean()}")
